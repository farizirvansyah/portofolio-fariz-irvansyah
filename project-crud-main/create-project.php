...
if (isset($_POST['save'])) {
  ...
  if ($image['error'] == 0) {
    ...
    //masukkan ke dalam projects sebutkan kolom di table projects nilainya diambil dari projects nginput
    if ($id) {
      //query update
      $update = mysqli_query($conn, "UPDATE projects SET title='$title', subtitle='$subtitle', link='$link', image='$filename' WHERE id='$id'");
      header("location:projects.php?update=berhasil");
    } else {
      $insert = mysqli_query($conn, "INSERT INTO projects (title, subtitle, link, image) 
            VALUES ('$title', '$subtitle', '$link', '$filename')");
      header("location:projects.php?tambah=berhasil");
    }
  }
  $update = mysqli_query($conn, "UPDATE projects SET title='$title', subtitle='$subtitle', link='$link' WHERE id='$id'");
  header("location:projects.php?update=berhasil");
}
?>
...
<head>
  ...
  <title>Management Projects - Create</title>
