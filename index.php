...
// User
$q_user = mysqli_query($conn, "SELECT * FROM users ORDER BY id DESC LIMIT 1");
$user = mysqli_fetch_assoc($q_user);
// Resume
$q_resume = mysqli_query($conn, "SELECT * FROM resume ORDER BY id DESC LIMIT 6");
...
if (isset($_POST['submit'])) {
	...
	// DML / SQL query untuk menyimpan data ke tabel contact
...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
						...
						<div class="row d-md-flex no-gutters slider-text align-items-end justify-content-end"
							...
							<div class="one-forth d-flex  align-items-center ftco-animate"
								...
								<div class="text">
									...
									<h1 class="mb-4 mt-3">
										<?= $title[0] . " " ?><span><?= array_shift($title);
											echo implode(" ", $title) ?></span>
									</h1>
									<h2 class="mb-4"><?= $v['subtitle'] ?></h2>
									<p class="mb-4 text-light"><?= $v['description'] ?></p>
									<p>
										<a href="<?= $v['button1_link'] ?>"
											class="btn btn-primary py-3 px-4"><?= $v['button1_text'] ?>
										</a>
										<a href="<?= $v['button2_link'] ?>"
											class="btn btn-white btn-outline-white py-3 px-4"><?= $v['button2_text'] ?>
										</a>
									</p>
								</div>
							</div>
					...
					<div class="row justify-content-start pb-3">
						<div class="col-md-12 heading-section ftco-animate">
							...
							<ul class="about-info mt-4 px-md-0 px-2">
								...
								<li class="d-flex"><span>Date of birth:</span> <span><?= $about['created_at'] ?></span>
								</li>
								<li class="d-flex"><span>Address:</span> <span><?= $about['address'] ?></span></li>
								<li class="d-flex">
									<span>Website Name:</span>
									<span>
										<a class="text-decoration-none" href="<?= $about['website_name'] ?>"
								...
								<li class="d-flex">
									<span>Instagram:</span>
									<span>
										<a class="text-decoration-none" href="<?= $about['ig'] ?>" target="_blank"
	...
	<section class="ftco-section ftco-no-pb" id="resume-section">
		<div class="container">
			...
			<div class="row">
				<?php
				// Membagi array resume menjadi 2 bagian secara otomatis untuk kolom kiri dan kanan
				$chunks = array_chunk($resume, ceil(count($resume) / 2));
				foreach ($chunks as $column):
					?>
					<div class="col-md-6">
						<?php foreach ($column as $vr): ?>
							<div class="resume-wrap ftco-animate">
								<span class="date"><?= htmlspecialchars($vr['year_start'] . " - " . $vr['year_end']) ?></span>
								<h2><?= htmlspecialchars($vr['title']) ?></h2>
								<span class="position"><?= htmlspecialchars($vr['subtitle']) ?></span>
								<p class="mt-4"><?= htmlspecialchars($vr['description']) ?></p>
							</div>
						<?php endforeach; ?>
					</div>
				<?php endforeach; ?>
				...
				foreach ($skills as $v) {
					...
					<div class="col-md-6 animate-box">
						<div class="progress-wrap ftco-animate">
							<h3 style="margin-bottom: <?= $v['progress'] < 7 ? '33px' : '5' ?>;"><?= $v['name'] ?></h3>
							<div class="progress">
								<div class="progress-bar color-1" role="progressbar" aria-valuenow="<?= $v['progress'] ?>"
